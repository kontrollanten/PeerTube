...
const { loadPlugin, runHook } = (() => {
  const hooks: Hooks = {}

  async function runHook<T> (hookName: ClientHookName, result?: T, params?: any) {
  ...
  function loadPlugin (options: {
    ...
    const { pluginInfo, peertubeHelpersFactory, formFields, onSettingsScripts } = options
