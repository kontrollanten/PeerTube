...
export class VideoUploadComponent extends VideoSend implements OnInit, AfterViewInit, CanComponentDeactivate {
  ...
  onUploadVideoOngoing (state: UploadState) {
    switch (state.status) {
      case 'queue':
        const nameWithoutExtension = state.name.replace(/\.[^/.]+$/, '')
        const name = nameWithoutExtension.length < 3 ? state.name : nameWithoutExtension
  ...
  ngOnInit () {
    ...
    this.uploadService.events
      .subscribe(
        state => this.isUploadingVideo && this.onUploadVideoOngoing(state),
        err => {
  ...
  getAudioUploadLabel () {
    ...
    if (this.isUploadingAudioFile) return $localize`Uploading...`
  ...
  uploadPreviewFile () {
    ...
    this.uploadService.events.subscribe((event: UploadState) => {
      console.log({event})

      if (event.name === 'complete') {
        this.uploadService.handleFiles(this.previewfileUpload, this.options)
      }
    })
  }
